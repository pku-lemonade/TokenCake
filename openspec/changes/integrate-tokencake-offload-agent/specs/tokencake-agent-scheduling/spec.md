...
The continued-execution optimization adds `reserve_generation_tokens=true`.
When enabled, logical admission commitments SHALL include the original prompt
plus the declared generation limit, clamped by native maximum model length,
and native speculative lookahead requirements. The default SHALL NOT allocate
future KV blocks eagerly. Setting it to false SHALL retain full-known-input
commitments for deployments that choose less conservative generation admission.
...
#### Scenario: Aggregate generation budgets exceed capacity

- **WHEN** two requests' current chunks fit but their combined input and declared generation budgets exceed uncommitted native capacity
- **THEN** the second admission waits while accepted work continues through incremental native allocation
- **AND** a resumed request's already generated output is not added twice to its original generation budget
...
### Requirement: Prefill chunking preserves native allowances

TokenCake SHALL preserve native long-prefill, token-budget, and model-length clamps. Backlog alone SHALL NOT impose the previous fixed 256-token cap. Aggregate logical admission SHALL constrain simultaneously admitted prefills without allocating their entire inputs early. An alternative adaptive chunk limit SHALL require complete-DAG performance evidence before adoption.

#### Scenario: Admit an annotated prefill under backlog

- **WHEN** an annotated request passes logical capacity admission while other requests are waiting
- **THEN** it retains the native per-step token allowance

#### Scenario: Constrain pressure through admission

- **WHEN** native physical capacity cannot cover a new request and admitted prefill commitments together
- **THEN** the new request remains pending while admitted work progresses through native allocation and necessary physical preemption

#### Scenario: Preserve the native allowance outside the cap

- **WHEN** the request is ordinary or TokenCake scheduling is disabled
...
The system SHALL apply TokenCake admission control to new admissions whenever a mixed workload has active or waiting annotated work. Admission SHALL account for the native coordinator's full-sequence multi-cache-group demand, outstanding admitted prefill growth, current shared capacity, applicable reserved capacity, and native physical capacity. These commitments SHALL NOT allocate future KV early or create another memory pool. Annotated reservation owners MAY consume their applicable reservation; ordinary requests SHALL use shared capacity only. A reservation denial SHALL leave new work pending. Already admitted work SHALL retain native allocation and physical-preemption progress, with growth beyond a revised partition charged as shared debt against future admissions. When all queued and running work is ordinary, the TokenCake admission path SHALL be bypassed.

#### Scenario: Prevent aggregate prefill overcommit

- **WHEN** two requests each fit individually but their complete prefill demands exceed currently available capacity together
- **THEN** the second remains pending and the first retains only its actually allocated chunk

#### Scenario: Change reservations while work is running

- **WHEN** a reservation change makes an admitted request exceed its former capacity share
- **THEN** the change constrains subsequent admissions without causing a reservation-only preemption of that request
...
#### Scenario: Compare recomputation cost within similar importance

- **WHEN** native allocation fails and multiple annotated victims have similar scores
- **THEN** victim selection considers recomputation tokens, actual releasable physical blocks, and near-completion protection without adding scores to milliseconds

#### Scenario: Avoid repeating an unchanged capacity failure

- **WHEN** a physically preempted request attempts readmission while competing work and usable capacity have not improved
- **THEN** it remains pending until capacity improves or competing work completes
